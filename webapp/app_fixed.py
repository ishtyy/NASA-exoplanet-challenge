...
# Load infographic data once at startup
...
try:
    ...
    INFOGRAPHIC_DATA = df_filtered[infographic_cols].to_dict(orient='records')
    print(f"✅ Infographic data loaded at startup. {len(INFOGRAPHIC_DATA)} records.")
except Exception as e:
    print(f"❌ Warning: Could not load infographic data at startup. {e}")
    INFOGRAPHIC_DATA = []
...
def home():
    ...
    print(f"✅ Home route called. Infographic data ready: {len(INFOGRAPHIC_DATA)} records.")
    return render_template('index.html')

@app.route('/api/infographic-data')
def get_infographic_data():
    """API endpoint to get infographic data as JSON."""
    return jsonify(INFOGRAPHIC_DATA)
