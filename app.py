...
def predict():
    ...
    try:
        data = request.get_json(force=True)  # force=True ensures JSON parsing even if headers are wrong
    except Exception:
        return jsonify({"error": "Invalid JSON payload."}), 400

    star_name = data.get('star_name')
    if not star_name:
        return jsonify({"error": "star_name is required."}), 400
...
# --- Error Handlers (so no HTML is returned) ---
